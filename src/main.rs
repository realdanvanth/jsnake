...
fn main(){
    ...
    let mut time:u64 = 500;
    loop{
        if eterm!(poll(time+1)){
            match read().unwrap(){
                Event::Key(event)=>{
        ...
        if !nextFrame(&mut snake,&dir,cols,rows,(&mut fx,&mut fy),&mut time)
                        {break;}
        display(&snake,(&fx,&fy));
        eterm!(move(0,0));
        //println!("{:?}",snake);
...
fn nextFrame(snake:&mut Vec<(u16,u16)>,dir:&Direction,cols:u16,rows:u16,food:(&mut u16,&mut u16),time:&mut u64)->bool{
